...
func manage( intention : Dictionary ) -> void:
	...
	if type == 'walk':
		if data[ 'type' ] == 'goto':
			...
			var step_length : float = data[ 'length' ] if data.has( 'length' ) else 2.0
			step( step_length )
	elif type == 'rotate':
		if data[ 'type' ] == 'direction':
			var direction : String = data[ 'direction' ]
			rotate_direction( direction )
		elif data[ 'type' ] == 'lookat':
			var target : String = data[ 'target' ]
			var id : int = data[ 'id' ] if data.has( 'id' ) else -1
			look_at_target( target, id )
	elif type == 'jump':
		do_jump()
	elif type == 'interact':
		if data[ 'type' ] == 'use':
...
func rotate_direction( direction : String ) -> void:
	...
	if direction == "left":
		...
		angle = PI
	# "forward" needs no rotation
	rotation.y += angle

func look_at_target( target_name : String, id : int ) -> void:
	var target_node = null
	if id != -1:
		target_node = instance_from_id( id )
	else:
		target_node = get_node_or_null( "/root/Root/NavigationRegion3D/Markers/" + target_name )
		if target_node == null:
			target_node = get_node_or_null( "/root/Root/NavigationRegion3D/Regions/" + target_name )
	if target_node != null:
